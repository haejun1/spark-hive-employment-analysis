...
# 데이터베이스 이름을 지정하지 않는 기본 풀 (데이터베이스 생성용)
_base_pool = PooledDB(
    creator=pymysql,
    maxconnections=2,
    blocking=True,
...
# 특정 데이터베이스에 연결하는 본진 풀 (크롤러 및 대량 적재용 ⭐️)
_pool = PooledDB(
    creator=pymysql,
    maxconnections=5,       # 최대 동시 연결을 5개로 제한 및 유지
    mincached=2,
    maxcached=5,
    blocking=True,
    host=os.getenv("DB_HOST"),
...
def get_base_connection():
    """데이터베이스 이름을 지정하지 않고 MySQL 서버 자체에 연결합니다."""
    return _base_pool.connection()

def get_connection():
    """설정된 특정 데이터베이스 세션을 풀에서 꺼내옵니다. (안전 무한 재사용)"""
